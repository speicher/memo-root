...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("鎮ㄩ€夋嫨鐨勬枃浠朵釜鏁拌秴杩囬檺鍒躲€俓n"+(message === 0 ? "鎮ㄥ凡杈惧埌涓婁紶鏂囦欢鐨勪笂闄愪簡銆�" : "鎮ㄨ繕鍙互閫夋嫨 " + message + " 涓枃浠�"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
		...
		switch (errorCode) {
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				progress.setStatus("鏂囦欢澶ぇ.");
				this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				progress.setStatus("涓嶈兘涓婁紶闆跺瓧鑺傛枃浠�.");
				this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				progress.setStatus("绂佹涓婁紶璇ョ被鍨嬬殑鏂囦欢.");
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("姝ｅ湪涓婁紶("+percent+"%)...");
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
				newTd.innerHTML = '鍥剧墖鎻忚堪<br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
				this.customSettings.imgBoxObj.appendChild(newTr);
			} else {
				showDialog('鍥剧墖涓婁紶澶辫触', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
			...
			if(data.aid) {
				...
				showDialog('涓婁紶澶辫触', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
